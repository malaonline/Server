...
$(function(){
    ...
    $('.save').click(function(){
        ...
        var dest = $in.attr('dest'), levelName = $.trim($row.find('td[field=name]').text());
        var gradeName = gradeId ? $('th[gradeId='+gradeId+']').text() : '所有年级';
        var subjectName = subjectId ? $('select[name="subject"]>option[value="'+subjectId+'"').text() : '所有科目';
        var decided = confirm('确定要修改"'+levelName+gradeName+subjectName+'"的' + destName[dest] + '为"' + _format_money(newVal) + '元/小时"吗?');
        if (!decided) return;
