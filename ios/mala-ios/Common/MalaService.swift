...
let parseOrderFormInfo: JSONDictionary -> OrderForm? = { orderInfo in
    ...
    if let
        ...
        // 订单信息
        let order = OrderForm(id: id, orderId: orderId, teacherId: teacher, teacherName: teacherName, avatarURL: avatar, schoolName: school, gradeName: grade, subjectName: subject, orderStatus: status, amount: amount, chargeChannle: chargeChannle, createAt: createdAt, evaluated: evaluated)
